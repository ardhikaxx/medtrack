@extends('layouts.app')

@section('title', 'Pencarian Global')

@section('breadcrumb')
<li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Beranda</a></li>
<li class="breadcrumb-item active">Pencarian</li>
@endsection

@section('content')
<div class="container-fluid">
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0"><i class="fas fa-search me-2"></i>Pencarian Global</h5>
                </div>
                <div class="card-body">
                    <div class="row mb-4">
                        <div class="col-md-6">
                            <div class="input-group">
                                <span class="input-group-text"><i class="fas fa-search"></i></span>
                                <input type="text" id="global-search" class="form-control" placeholder="Ketik untuk mencari..." autofocus>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <select id="search-type" class="form-select">
                                <option value="all">Semua Kategori</option>
                                <option value="pasien">Pasien</option>
                                <option value="rekam_medis">Rekam Medis</option>
                                <option value="peminjaman">Peminjaman</option>
                                <option value="pengguna">Pengguna</option>
                            </select>
                        </div>
                    </div>

                    <div id="search-results">
                        <div class="text-center text-muted py-5">
                            <i class="fas fa-search fa-3x mb-3 opacity-50"></i>
                            <p>Masukkan kata kunci untuk memulai pencarian</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

@push('scripts')
<script>
$(document).ready(function() {
    let debounceTimer;
    
    function performSearch(query, type) {
        if (query.length < 2) {
            $('#search-results').html(`
                <div class="text-center text-muted py-5">
                    <i class="fas fa-search fa-3x mb-3 opacity-50"></i>
                    <p>Masukkan kata kunci untuk memulai pencarian</p>
                </div>
            `);
            return;
        }

        $.get('{{ route("search.results") }}', { q: query, type: type })
            .done(function(data) {
                let html = '';
                
                if (data.pasien.length === 0 && data.rekam_medis.length === 0 && 
                    data.peminjaman.length === 0 && data.pengguna.length === 0) {
                    html = `
                        <div class="text-center text-muted py-5">
                            <i class="fas fa-folder-open fa-3x mb-3 opacity-50"></i>
                            <p>Tidak ada hasil yang ditemukan</p>
                        </div>
                    `;
                } else {
                    if (data.pasien.length > 0) {
                        html += '<h6 class="text-uppercase text-muted mb-3">Pasien (' + data.pasien.length + ')</h6>';
                        html += '<div class="list-group mb-4">';
                        data.pasien.forEach(function(item) {
                            html += `
                                <a href="/pasien/${item.id}" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center">
                                    <div>
                                        <div class="fw-semibold">${item.nama}</div>
                                        <small class="text-muted">No. RM: ${item.no_rm}</small>
                                    </div>
                                    <span class="badge bg-primary rounded-pill"><i class="fas fa-user"></i></span>
                                </a>
                            `;
                        });
                        html += '</div>';
                    }

                    if (data.rekam_medis.length > 0) {
                        html += '<h6 class="text-uppercase text-muted mb-3">Rekam Medis (' + data.rekam_medis.length + ')</h6>';
                        html += '<div class="list-group mb-4">';
                        data.rekam_medis.forEach(function(item) {
                            html += `
                                <a href="/rekam-medis/${item.id}" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center">
                                    <div>
                                        <div class="fw-semibold">${item.no_rekam_medis}</div>
                                        <small class="text-muted">${item.nama_pasien || '-'}</small>
                                    </div>
                                    <span class="badge bg-info rounded-pill"><i class="fas fa-file-medical"></i></span>
                                </a>
                            `;
                        });
                        html += '</div>';
                    }

                    if (data.peminjaman.length > 0) {
                        html += '<h6 class="text-uppercase text-muted mb-3">Peminjaman (' + data.peminjaman.length + ')</h6>';
                        html += '<div class="list-group mb-4">';
                        data.peminjaman.forEach(function(item) {
                            html += `
                                <a href="/peminjaman/${item.id}" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center">
                                    <div>
                                        <div class="fw-semibold">${item.no_peminjaman}</div>
                                        <small class="text-muted">${item.nama_peminjam_luar || '-'} - ${item.institusi_peminjam || '-'}</small>
                                    </div>
                                    <span class="badge bg-warning rounded-pill"><i class="fas fa-handshake"></i></span>
                                </a>
                            `;
                        });
                        html += '</div>';
                    }

                    if (data.pengguna.length > 0) {
                        html += '<h6 class="text-uppercase text-muted mb-3">Pengguna (' + data.pengguna.length + ')</h6>';
                        html += '<div class="list-group mb-4">';
                        data.pengguna.forEach(function(item) {
                            html += `
                                <a href="/pengguna/${item.id}" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center">
                                    <div>
                                        <div class="fw-semibold">${item.nama_lengkap}</div>
                                        <small class="text-muted">${item.email}</small>
                                    </div>
                                    <span class="badge bg-success rounded-pill"><i class="fas fa-user-cog"></i></span>
                                </a>
                            `;
                        });
                        html += '</div>';
                    }
                }
                
                $('#search-results').html(html);
            })
            .fail(function() {
                $('#search-results').html(`
                    <div class="text-center text-danger py-5">
                        <i class="fas fa-exclamation-triangle fa-3x mb-3"></i>
                        <p>Terjadi kesalahan saat mencari</p>
                    </div>
                `);
            });
    }

    $('#global-search').on('input', function() {
        clearTimeout(debounceTimer);
        const query = $(this).val();
        const type = $('#search-type').val();
        debounceTimer = setTimeout(function() {
            performSearch(query, type);
        }, 300);
    });

    $('#search-type').change(function() {
        const query = $('#global-search').val();
        performSearch(query, $(this).val());
    });
});
</script>
@endpush
